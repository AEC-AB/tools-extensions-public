...
| Tool | Use |
| --- | --- |
| `extensions-get-available-templates` | List extension templates before choosing a scaffold. |
| `extensions-create-from-template` | Create an extension project from a selected template. |

## Project understanding and local testing

| Tool | Use |
| --- | --- |
| `extensions-get-project-metadata` | Read a local project's metadata, schema, and expected arguments. |
| `extensions-run-project` | Run a local extension project with structured inputs. |
| `run-csharp-script` | Run a C# script in Revit on the Revit UI thread against the active document. |

## Registered extensions

| Tool | Use |
| --- | --- |
| `extensions-search` | Search available extension packages for reuse or examples. |
| `extensions-get-metadata` | Read metadata for a registered extension by ID. |
| `extensions-run` | Run a registered extension by ID. |
| `extension-register-from-project` | Register a private extension from a local project for testing. |
| `extension-publish-from-project` | Build, version, and publish a local extension project. |

## Typed operations

| Tool | Use |
| --- | --- |
| `search_operations` | Find typed operations for a capability. |
| `get_operations` | Read selected operation schemas, inputs, outputs, and relationships. |
| `run_operation` | Run an inspected operation with a JSON configuration payload. |
